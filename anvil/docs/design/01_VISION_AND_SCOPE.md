...
| G2 | **Multi-genre** via modules (card → topdown → vn → shmup → fps2 → net later) |
| G3 | **Interactive verification** built-in (test, observe, replay-ready) |
...
## 6. Success definition (product)

Anvil is “ready for real games” when:
...
Then (and only then) unpark titles like Gravewake.
